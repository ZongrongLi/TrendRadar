...
### 5️⃣ 第五步：远程云存储配置
   ...
   GitHub Actions 会按 `.github/workflows/crawler.yml` 中的 `schedule` 持续运行，不再需要额外签到续期。

   - **运行方式**：根据 `cron` 配置定时执行，也支持在 Actions 页面手动触发 `Get Hot News`
   - **修改频率**：编辑 `.github/workflows/crawler.yml` 中的 `cron: "..."` 即可
   - **注意时区**：GitHub Actions 使用 **UTC** 时间；如果你想按北京时间运行，需要先减去 8 小时
